...
num_male = num_all/2;
R = 20;
Res = myPCA(data,R);                  % for all
% Res = myPCA(data(:,1:num_male),R);    % for male
% Res = myPCA(data(:,num_male+1:num_all),R);  % for female
...
hold on;
plot(x,0.85*ones(1,R),'--');
ylabel('Cumulative contribution rate of principal components');ylim([0 1.08]);
title(['Contribution Rate Analysis of PCA (Gender) (R=',num2str(R),')']);
...
plot(x,0.85*ones(1,R),'--');
ylabel('Cumulative contribution rate of principal components');ylim([0 1.08]);
title(['Contribution Rate Analysis of PCA (MNIST) (R=',num2str(R),')']);
xlabel('The first k principal component');
xlim([0 R+1]);
